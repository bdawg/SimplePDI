...
if doDerotate:
            ...
            for c in range(2):
                for l in range(2):
                    ...
                    if c == 0:
                        ...
                        curImIn = ndimage.interpolation.rotate(np.fliplr(curImIn), bsRot, mode='nearest',
                                                               reshape=False)
    ...
    allSummedIms = allSummedImsOrig # Currently not working, lots of stuff commented out!


# Now make a set of Stokes images and derotate them
count = 0
allIms = []
for ind in imInd:
    print "ind: %d" % ind
    h0c0l0 = allSummedIms[:, :, ind, 0, 0, 0]
    h0c0l1 = allSummedIms[:, :, ind, 0, 0, 1]
    h0c1l0 = allSummedIms[:, :, ind, 0, 1, 0]
    h0c1l1 = allSummedIms[:, :, ind, 0, 1, 1]
    h1c0l0 = allSummedIms[:, :, ind, 1, 0, 0]
    h1c0l1 = allSummedIms[:, :, ind, 1, 0, 1]
    h1c1l0 = allSummedIms[:, :, ind, 1, 1, 0]
    h1c1l1 = allSummedIms[:, :, ind, 1, 1, 1]
    h2c0l0 = allSummedIms[:, :, ind, 2, 0, 0]
    h2c0l1 = allSummedIms[:, :, ind, 2, 0, 1]
    h2c1l0 = allSummedIms[:, :, ind, 2, 1, 0]
    h2c1l1 = allSummedIms[:, :, ind, 2, 1, 1]
    h3c0l0 = allSummedIms[:, :, ind, 3, 0, 0]
    h3c0l1 = allSummedIms[:, :, ind, 3, 0, 1]
    h3c1l0 = allSummedIms[:, :, ind, 3, 1, 0]
    h3c1l1 = allSummedIms[:, :, ind, 3, 1, 1]
...
curHDU = fits.PrimaryHDU()
curHDU.data = im
curHDU.writeto('outa.fits', clobber=True)
