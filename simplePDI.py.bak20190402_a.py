...
class pdiImages:
    def __init__(self, outDir='./', showPlots=True):
        ...
        self.allQualityStats = None


    def loadCube(self, dataPath, loadFilename, twoCamMode = True):
        npzfile = np.load(dataPath+loadFilename)
        self.allSummedImsOrig = npzfile['arr_0']
        self.allPAsOrig = npzfile['arr_1']
        try:
            self.allQualityStats = npzfile['arr_2']
        except:
            print('No quality stats found in cube file.')
        del (npzfile)
    ...
    def checkInputdata(self, thresh=15000):
        pass
    ...
    def processRawFiles(self, dataPath, filePref, nSubFiles, startFileNum = 0,
        ...
        if singleFileOnly:
            nSubFiles=16
        allSummedIms = np.zeros([dim, dim, nSubFiles // 16, 4, 2, 2])
        allQualityStats = np.zeros([3, nSubFiles // 16, 4, 2, 2]) # pk, total, l2 flux
        ...
        for f in range(0, nSubFiles // 4):
            ...
            for c in range(0, 2):
                ...
                for l in range(0, 2):
                    ...
                    allSummedIms[:, :, curHWPSet, hwpState, c, l] = curCubeShiftedSummed
                    pkflux = np.amax(curCubeShiftedSummed)
                    totflux = np.sum(curCubeShiftedSummed)
                    l2flux = np.sum(curCube ** 2)
                    allQualityStats[:, curHWPSet, hwpState, c, l] = [pkflux, totflux, l2flux]
            ...
            print('Time for set of 2 cam files: %f seconds.' % elTime)
        ...
        if saveCube:
            ...
            saveFilename = savePath + saveFilePref + filePref
            print('Saving processed cube as '+saveFilename)
            np.savez(saveFilename, allSummedIms, pas, allQualityStats)

        self.allSummedImsOrig = allSummedIms
        self.allPAsOrig = pas
        self.allQualityStats = allQualityStats



    def plotQualityStats(self, fignum=1):
        pkflux = self.allQualityStats[0, :, :, 0, 0].ravel()
        totflux = self.allQualityStats[1, :, :, 0, 0].ravel()
        l2flux = self.allQualityStats[2, :, :, 0, 0].ravel()
        plt.figure(fignum)
        plt.subplot(311)
        plt.plot(pkflux)
    ...
    def makeDiffIms(self, doMask = False, maskRad = 2., maskVal = 1., clim = [], deRotate = True,
        ...
        for ind in imInd:
            ...
            for h in range(4):
                pa = allPAs[ind, h]
                # if reversePA:
                #     pa = -pa  # To address image rotator minus-sign bug
                curPaSet.append(pa)
                ...
                for c in range(2):
                    for l in range(2):
                        ...
                        if maxPixThresh is not None:
                            maxPix = np.max(curImIn.ravel())
                            # print('Max pixel value: %f' % maxPix)
                            allMaxVals.append(maxPix)
                            if maxPix > maxPixThresh:
                                print('WARNING! Image %d,%d,%d,%d may be saturated - max value of %f' %
                                      (ind, h, c, l, maxPix))
    ...
    def plotStokesIms(self, data = None, index = None, fignum = 2, clim = None,
        ...
        if self.showPlots:
            ...
            for k in range(6):
                ...
                if k == 3:# & (climp is not None):
                    curClim = climp
                elif k == 4:# & (climpQ is not None):
                    curClim = climpQ
                elif k == 5:# & (climpU is not None):
                    curClim = climpU
                else:
                    curClim = clim
    ...
    class sortPolzState:
        def __init__(self, dataIn, ind):
            ...
            # # TESTING! Swap the cameras
            # self.h0c0l0 = dataIn[:, :, ind, 0, 1, 0]
            # self.h0c0l1 = dataIn[:, :, ind, 0, 1, 1]
            # self.h0c1l0 = dataIn[:, :, ind, 0, 0, 0]
            # self.h0c1l1 = dataIn[:, :, ind, 0, 0, 1]
            # self.h1c0l0 = dataIn[:, :, ind, 1, 1, 0]
            # self.h1c0l1 = dataIn[:, :, ind, 1, 1, 1]
            # self.h1c1l0 = dataIn[:, :, ind, 1, 0, 0]
            # self.h1c1l1 = dataIn[:, :, ind, 1, 0, 1]
            # self.h2c0l0 = dataIn[:, :, ind, 2, 1, 0]
            # self.h2c0l1 = dataIn[:, :, ind, 2, 1, 1]
            # self.h2c1l0 = dataIn[:, :, ind, 2, 0, 0]
            # self.h2c1l1 = dataIn[:, :, ind, 2, 0, 1]
            # self.h3c0l0 = dataIn[:, :, ind, 3, 1, 0]
            # self.h3c0l1 = dataIn[:, :, ind, 3, 1, 1]
            # self.h3c1l0 = dataIn[:, :, ind, 3, 0, 0]
            # self.h3c1l1 = dataIn[:, :, ind, 3, 0, 1]
